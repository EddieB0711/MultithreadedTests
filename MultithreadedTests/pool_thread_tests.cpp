...
auto __stdcall run_long_process(PTP_CALLBACK_INSTANCE, void * args, PTP_WORK) -> void
{
	...
	for (auto i = 0; i < upper_bound; ++i)
		for (auto j = 0; j < lower_bound; ++j); // some process that takes time.
}
...
auto run_pool_threaded_tests(bounded_iterations current_iterations, thread_environment & thread, p_log_results const & results_logger) -> void
{
	auto work = std::vector<multi_threaded_work> { };

	for (auto i = 1; i <= current_iterations.work(); ++i) work.push_back(multi_threaded_work { i, current_iterations }); // create work.
...
auto pool_thread_tests::run_test() const -> void
{
	...
	auto && thread = multi_threading::thread_environment { };

	for (auto const & iter : iterations) run_pool_threaded_tests(create_bounded_iterations(iter), thread, m_results_logger);
}